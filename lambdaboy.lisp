...
(defun log-op (gb opcode opstr &rest args)
  (let ((reg (gameboy-register gb)))
    ...
    (flet ((operand-1 ()
      ...
      (let ((pc-diff
              (4bit-case (op-ms4 op-ls4)
                ((#x0 #x0)
                 (log-op gb opcode "NOP")
                 0)
                ((#x1 #x8)
                 (let ((offset (i8-as-integer (operand-1))))
                   (log-op gb opcode "JR #x~x" offset)
                   (incf (register-pc reg) (1+ offset))
                   0))
                ((#x2 #x0)
                 (let ((offset (i8-as-integer (operand-1))))
                   (log-op gb opcode "JR NZ, #x~x" offset)
                ...
                ((#x2 #x8)
                 (let ((offset (i8-as-integer (operand-1))))
                   (log-op gb opcode "JR Z, #x~x" offset)
                ...
                ((#xc #x3)
                 (let ((a16 (8bit->16bit (operand-1) (operand-2))))
                   (log-op gb opcode "JP #x~x" a16)
                   (setf (register-pc reg) a16))
                 0)
                ((#xc #x8)
                 (log-op gb opcode "RET Z")
                   ...
                   (flet ((reset-bit (n)
                            (let ((n (+ n (if (zerop (logand #x8 op-ls4))
                              ...
                              (multiple-value-bind (name val)
                                ...
                                (log-op gb opcode "RES ~a, ~a" n name)
                          ...
                          (set-bit (n)
                            (let ((n (+ n (if (zerop (logand #x8 op-ls4))
                              ...
                              (multiple-value-bind (name val)
                                  (select-register idx (b c d e h l (hl) a))
                                (log-op gb opcode "SET ~a, ~a" n name)
                                (set-register idx (b c d e h l (hl) a)
                                              (logior val (ash 1 n)))))))
                     (4bit-case (op-ms4 op-ls4)
                       ((#x3 _)
                       ...
                       ((#xf _) (set-bit 6)
                                0)
                       ((#xe #x6)
                        (log-op gb opcode "SET 4, (HL)")
                        (setf (register-hl reg)
                              (logand (register-hl reg) #.(lognot #x0008)))
                        0)))))
                ((#xc #xd)
                 (let ((a16 (8bit->16bit (operand-1) (operand-2))))
                   (log-op gb opcode "CALL #x~x" a16)
                   (op-call reg mem a16))
                 0)
                ((#xe #x0)
                 (log-op gb opcode "LDH (#x~x), A" (operand-1))
                 (setf (memory-address mem (+ #xff00 (operand-1)))
                       (register-a reg))
                 1)
                ((#xe #xa)
                 (let ((a16 (8bit->16bit (operand-1) (operand-2))))
                   (log-op gb opcode "LD (#x~x), A" a16)
                   (setf (memory-address mem a16) (register-a reg))
                   2))
                ((#xf #x0)
                 (log-op gb opcode "LDH A, (#x~x)" (operand-1))
                ...
                ((#xf #xa)
                 (let ((a16 (8bit->16bit (operand-1) (operand-2))))
                   (log-op gb opcode "LD A, #x~x" a16)
                   (setf (register-a reg) a16))
                 2)
                ((#xf #xe)
                 (log-op gb opcode "CP #x~x " (operand-1))
                ...
                ((#x3 #x8)
                 (let ((offset (i8-as-integer (operand-1))))
                   (log-op gb opcode "JR C, #x~x" offset)
                ...
                ((#x4 _)
                 (multiple-value-bind (name val)
                   ...
                   (if (zerop (logand #x8 op-ls4))
                       (progn
                         (log-op gb opcode "LD B, ~a" name)
                         (setf (register-b reg) val))
                       (progn
                         (log-op gb opcode "LD C, ~a" name)
                ...
                ((#x5 _)
                 (multiple-value-bind (name val)
                   ...
                   (if (zerop (logand #x8 op-ls4))
                       (progn
                         (log-op gb opcode "LD D, ~a" name)
                         (setf (register-d reg) val))
                       (progn
                         (log-op gb opcode "LD E, ~a" name)
                ...
                ((#x6 _)
                 (multiple-value-bind (name val)
                   ...
                   (if (zerop (logand #x8 op-ls4))
                       (progn
                         (log-op gb opcode "LD H, ~a" name)
                         (setf (register-h reg) val))
                       (progn
                         (log-op gb opcode "LD L, ~a" name)
                ...
                ((#x7 _)
                 (if (zerop (logand #x8 op-ls4))
                     (if (= op-ls4 #x6)
                         (progn
                           (log-op gb opcode "HALT")
                           (error "halt"))
                         (multiple-value-bind (name val)
                             (select-register (logand #x7 op-ls4)
                                              (b c d e h l (hl) a))
                           (log-op gb opcode "LD (HL), ~a" name)
                           (setf (memory-address mem (register-hl reg)) val)))
                     (multiple-value-bind (name val)
                         (select-register (logand #x7 op-ls4)
                                          (b c d e h l (hl) a))
                       (log-op gb opcode "LD A, ~a" name)
                ...
                ((#xb _)
                 (multiple-value-bind (name val)
                   ...
                   (if (zerop (logand #x8 op-ls4))
                       (progn
                         (log-op gb opcode "OR ~a" name)
                         (let ((result (logior (register-a reg) val)))
                           (setf (register-a reg) result)
                           (set-flags :zero (zerop result) :sub nil :hc nil :carry nil)))
                       (progn
                         (log-op gb opcode "CP ~a" name)
                         (let ((result (- (register-a reg) val)))
                           (set-flags :zero (zerop result)
                                      :sub t
                                      :hc (> result #x0f)
                 ...
                 (if (<= op-ms4 #x3)
                     (let ((d16 (8bit->16bit (operand-1) (operand-2))))
                       (let ((name (set-register op-ms4 (bc de hl sp) d16)))
                         (log-op gb opcode "LD ~a, #x~x" name d16)
                         2))
                ...
                ((_ #x6)
                 (if (<= op-ms4 #x3)
                     (let ((name (set-register op-ms4 (b d h (hl)) (operand-1))))
                       (log-op gb opcode "LD ~a, #x~x" name (operand-1))
                       2)
                     (case op-ms4
                       (#xc (log-op gb opcode "ADD #x~x" (operand-1))
                            (let ((result (+ (register-a reg) (operand-1))))
                              (set-flags :zero (zerop result)
                                         :sub nil
                                         :hc (< result #x0f)
                                         :carry (> result #xff))
                              (setf (register-a reg) result)))
                       (#xd (log-op gb opcode "SUB #x~x" (operand-1))
                            (let ((result (- (register-a reg) (operand-1))))
                              (set-flags :zero (zerop result)
                                         :sub t
                                         :hc (> result #x0f)
                ...
                ((_ #x7)
                 (if (<= op-ms4 #x3)
                     ...
                     (let ((addr (ecase op-ms4
                                   ...
                                   (#xe #x20)
                                   (#xf #x30))))
                       (log-op gb opcode "RST #x~2,'0x" addr)
                ...
                ((_ #x9)
                 (if (<= op-ms4 #x3)
                     (multiple-value-bind (name val)
                         (select-register op-ms4 (bc de hl sp))
                       (log-op gb opcode "ADD HL, ~a" name)
                ...
                ((_ #xa)
                 (if (<= op-ms4 #x3)
                     (multiple-value-bind (name val)
                       ...
                       (cond ((= op-ms4 #x2)
                              (log-op gb opcode "LD A, (HL+)")
                              (incf (register-hl reg)))
                             ((= op-ms4 #x3)
                              (log-op gb opcode "LD A, (HL-)")
                              (incf (register-hl reg) -1))
                             (t (log-op gb opcode "LD A, ~a" name)))
                       (setf (register-a reg) val)
                       0)
                     (unknown-instruction)))
                ((_ #xb)
