      ...
      (let ((pc-diff
              (case opcode
                ...
                (#x0e (log-op "LD C, #x~x" (operand-1))
                      ...
                      2)
                (#x11 (let ((d16 (8bit->16bit (operand-1) (operand-2))))
                        (log-op "LD DE, #x~x" d16)
                        (setf (register-de reg) d16))
                      3)
                (#x18 (let ((offset (i8-as-integer (operand-1))))
                        (log-op "JR #x~x" offset)
                        (incf (register-pc reg) offset)
                        0))
                (#x19 (log-op "ADD HL, DE")
                      (let ((result (+ (register-hl reg) (register-de reg))))
                        (setf (register-hl reg) result)
                        (set-flags :sub nil :hc (> result #x0f) :carry (> result #xffff))
                        1))
                ...
                (#xe0 (log-op "LDH #x~x, A" (operand-1))
                      (setf (memory-address mem (+ #xff00 (operand-1)))
                            (register-a reg))
                      2)
                (#xea (let ((a16 (8bit->16bit (operand-1) (operand-2))))
                        (log-op "LD #x~x, A" a16)
